...
# - A fixed-effects participant-bootstrap sensitivity analysis is fitted using
#   the same resampled participant clusters.
# ================================================================
...
fit_domain_models_fixed <- function(dat) {
  fit_aperiodic <- lm(
    aperiodic_score_z ~ fw_z + chi_z + age_z + led_z +
      global_fw_z + global_chi_z,
    data=dat
  )
  fit_burst <- lm(
    burst_score_z ~ fw_z + chi_z + age_z + led_z +
      global_fw_z + global_chi_z,
    data=dat
  )

  b_ap <- coef(fit_aperiodic)
  b_burst <- coef(fit_burst)
  fw_difference <- b_ap["fw_z"] - b_burst["fw_z"]
  chi_difference <- b_ap["chi_z"] - b_burst["chi_z"]

  tibble(
    fixed_fw_aperiodic=unname(b_ap["fw_z"]),
    fixed_fw_burst=unname(b_burst["fw_z"]),
    fixed_chi_aperiodic=unname(b_ap["chi_z"]),
    fixed_chi_burst=unname(b_burst["chi_z"]),
    fixed_fw_domain_difference=unname(fw_difference),
    fixed_chi_domain_difference=unname(chi_difference),
    fixed_double_dissociation=unname(fw_difference-chi_difference)
  )
}
...
observed_dd_fixed <- fit_domain_models_fixed(dd_wide)

cat("\nObserved double-dissociation estimates:\n")
print(observed_dd)
cat("\nObserved fixed-effects sensitivity estimates:\n")
print(observed_dd_fixed)
...
write_csv(
  observed_dd_fixed,
  file.path(bootstrap_dir,"observed_fixed_effects_sensitivity_estimates.csv")
)
...
bootstrap_once <- function(iteration) {
  ...
  tryCatch({
    mixed_result <- fit_domain_models(boot_dat)
    fixed_result <- fit_domain_models_fixed(boot_dat)
    bind_cols(mixed_result,fixed_result) %>%
      mutate(
        iteration = iteration,
        status = "OK"
      )
    ...
    error = function(e) {
      tibble(
        ...
        fixed_fw_aperiodic = NA_real_,
        fixed_fw_burst = NA_real_,
        fixed_chi_aperiodic = NA_real_,
        fixed_chi_burst = NA_real_,
        fixed_fw_domain_difference = NA_real_,
        fixed_chi_domain_difference = NA_real_,
        fixed_double_dissociation = NA_real_,
        iteration = iteration,
        status = paste(
          "ERROR:",
          conditionMessage(e)
...
bootstrap_summary <- successful_bootstrap %>%
  ...
  mutate(
    ci_excludes_zero =
      ci_lower_95 > 0 |
      ci_upper_95 < 0,
    two_sided_bootstrap_tail_proportion=pmin(
      1,
      2*pmin(proportion_at_or_below_zero,proportion_at_or_above_zero)
    )
  )
...
summarise_bootstrap_parameter <- function(parameter_name) {
  ...
  tibble(
    ...
    proportion_same_direction=mean(
      sign(values)==sign(observed_value),na.rm=TRUE
    ),
    two_sided_bootstrap_tail_proportion=pmin(1,2*pmin(lower_tail,upper_tail)),
    ci_excludes_zero = (
      ci[1] > 0 ||
      ci[2] < 0
    )
...
# Fixed-effects participant-bootstrap sensitivity reported in the manuscript.
fixed_values <- successful_bootstrap$fixed_double_dissociation
fixed_ci <- quantile(fixed_values,c(.025,.975),na.rm=TRUE,names=FALSE)
fixed_lower_tail <- mean(fixed_values <= 0,na.rm=TRUE)
fixed_upper_tail <- mean(fixed_values >= 0,na.rm=TRUE)

fixed_effects_summary <- tibble(
  parameter="fixed_double_dissociation",
  observed=observed_dd_fixed$fixed_double_dissociation[[1]],
  n_successful=sum(!is.na(fixed_values)),
  bootstrap_mean=mean(fixed_values,na.rm=TRUE),
  bootstrap_se=sd(fixed_values,na.rm=TRUE),
  ci_lower_95=fixed_ci[1],
  ci_upper_95=fixed_ci[2],
  proportion_at_or_below_zero=fixed_lower_tail,
  proportion_at_or_above_zero=fixed_upper_tail,
  two_sided_bootstrap_tail_proportion=pmin(
    1,2*pmin(fixed_lower_tail,fixed_upper_tail)
  )
)

cat("\nFixed-effects participant-bootstrap sensitivity:\n")
print(fixed_effects_summary)
write_csv(
  fixed_effects_summary,
  file.path(bootstrap_dir,"participant_bootstrap_fixed_effects_summary.csv")
)
...
cat(
  ...
  " - participant_bootstrap_summary.csv\n",
  " - participant_bootstrap_parameter_summary.csv\n",
  " - participant_bootstrap_fixed_effects_summary.csv\n",
  " - participant_bootstrap_all_iterations.csv\n",
  " - participant_bootstrap_failed_iterations.csv\n",
  sep = ""
)
